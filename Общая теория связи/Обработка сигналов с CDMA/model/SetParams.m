...
function Encoder = SetParamsEncoder(inEncoder, ParamsNumber, ...
    ...
    % Нужно ли выполнять кодирование и декодирование
        ...
        if ~isfield(Encoder, 'TypeModulation')
            Encoder.TypeModulation = NaN;
        else
            % Проверка корректности введённых значений
        end
        ...
        if ~isfield(Encoder, 'DecisionMethod')
            Encoder.DecisionMethod = 'Approximate log-likelihood ratio';
        else
            if ~(strcmp(Encoder.DecisionMethod, 'Hard decision') || ...
                    strcmp(Encoder.DecisionMethod, ...
...
function Mapper = SetParamsMapper(inMapper, ParamsNumber, ...
    ...
    % Выходные данные виде битов или нет
        if ~isfield(Mapper, 'BitOutput')
            Mapper.BitOutput = true;
        else
            % Проверка корректности введённых значений
        end
    ...
    % биты для мягкого решения
        if ~isfield(Mapper, 'NumSoftBits')
            Mapper.NumSoftBits = 4;
        else
            % Проверка корректности введённых значений
        end
end
function OFDM = SetParamsOFDM(inOFDM, ParamsNumber, ...
    LogLanguage) %#ok<INUSD,DEFNU>

    % Пересохраним входные данные
        OFDM = inOFDM;

    % Нужно ли пропускать сигнал через OFDM 
        if ~isfield(OFDM, 'isTransparent')
            OFDM.isTransparent = true;
        else
            % Проверка корректности введённых значений
        end
        
    % используется ли эквалайзер, по умолчанию нет
        if ~isfield(OFDM, 'isEQ')
            OFDM.isEQ = true;
        else
            % Проверка корректности введённых значений
        end
        
    % длина циклического префикла
        if ~isfield(OFDM, 'CP')
            OFDM.CP = 1/8;
        else
            % Проверка корректности введённых значений
    ...
    % метод интерполяции 
        if ~isfield(OFDM, 'InterpMethod')
            OFDM.InterpMethod = 'linear';
        else
            % Проверка корректности введённых значений
        end

    % тип эквалайзера
        if ~isfield(OFDM, 'TypeEQ')
            OFDM.TypeEQ = 'ZF';
        else
            % Проверка корректности введённых значений
        end
end
function CDMA = SetParamsCDMA(inCDMA, ParamsNumber, ...
    LogLanguage) %#ok<INUSD,DEFNU>
    ...
    % Нужно ли пропускать сигнал через OFDM 
        if ~isfield(CDMA, 'isTransparent')
            CDMA.isTransparent = true;
        else
            % Проверка корректности введённых значений
        end
        
    % используется ли эквалайзер, по умолчанию нет
        if ~isfield(CDMA, 'isNOTEQ')
            CDMA.isNOTEQ = true;
        else
            % Проверка корректности введённых значений
        end
    ...
    % нужен ли скремблинг
        if ~isfield(CDMA, 'isNOTScramb')
            CDMA.isNOTScramb = true;
        else
            % Проверка корректности введённых значений
        end

    % коэф расширения
        if ~isfield(CDMA, 'Expansion')
            CDMA.Expansion = 256;
        else
            % Проверка корректности введённых значений
        end
    ...
    % случайное число
        if ~isfield(CDMA, 'ChannelizationCodeId')
            CDMA.ChannelizationCodeId = 10;
        else
            % Проверка корректности введённых значений
        end

    % тип эквалайзера
        if ~isfield(CDMA, 'RayCount')
            CDMA.RayCount = 1;
        else
            % Проверка корректности введённых значений
        end
...
function Channel = SetParamsChannel(inChannel, ParamsNumber, ...
    ...
    % максимальный Допплеровский сдвиг в Гц
        if ~isfield(Channel, 'DopplerShift')
            Channel.DopplerShift = 0;
        else
            % Проверка корректности введённых значений
        end

    % максимальный Допплеровский сдвиг в Гц
        if ~isfield(Channel, 't_shift')
            Channel.t_shift = 0;
        else
            % Проверка корректности введённых значений
        end
...
function BER = SetParamsBER(inBER, ParamsNumber, ...
    ...
    % будут остановлены
        if ~isfield(BER, 'MinFER')
            BER.MinFER = 100; % т.е. ограничения на это значение нет!
        else
            % Проверка корректности введённых значений
        end
